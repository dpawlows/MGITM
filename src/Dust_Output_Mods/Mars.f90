...
! Mods 11-11-14 : s. W. Bougher
!  if(photodis(1:NWH,iCO2_) .lt. 0.) photodis(1:NWH,iCO2_) = 0.  (ON)
! Mods 11-11-15 : s. W. Bougher
!  if(photodis(1:NWH,iCO2_) .lt. 0.) photodis(1:NWH,iCO2_) = 0.  (OFF)
! Mods 11-11-16 : D. Pawlowski
! photoabs(58,iCO2_) = 0.0!photoabs(38:,iCO2_) * .1
! Mods 11-12-01:   S. BOUGHER
! E(EUV) = 0.20 (average of altitude variation)
  ...
  !   photodis(1:NWH-7,iCO2_)  = PhotoAbs_CO2(1:NWH-7)-PhotoIon_CO2(1:NWH-7)
  !   photodis(1:NWH,iCO2_)  = PhotoAbs_CO2(1:NWH)-PhotoIon_CO2(1:NWH)
  !   do i=1,NWH
  !      if(photodis(NWH,iCO2_) .lt. 0.) photodis(NWH,iCO2_) = 0.
  !   enddo
  photodis(1:NWH,iCO2_)  = PhotoAbs_CO2(1:NWH)-PhotoIon_CO2(1:NWH)
  ...
  if( floor((tSimulation - dT)/DtLTERadiation) == &
     ...
     do iLat = 1, nLats
        do iLon = 1, nLons
           ...
           ! Determining the top of the lower atmosphere radiative zone (near -IR)
           ! May need refinement to better comform to Table 1 of LV(1998)
           do iAlt=1,nAlts
...
subroutine calc_radcooling(iBlock)
  ...
  !  Diagnostics:  Pressure, Temperature, VMRO1, VMRCO2
  !  =======================================================================
  !
...
!     write(*,*) GWAccel(1,1,:,1)
  ...
  subroutine  calc_lowatmosrad(iblock,iLat,iLon,L_LAYERS,L_LEVELS,&
    ...
    real :: FMNETV(LL_NLAYRAD), diffvt,tautot, tDiff(nDustTimes),ctDiff(nDustTimes),conrnu
...
!stop
 if (UseDustDistribution) then
       ...
       conrathrtime = (CurrentTime - Conrathtime(cmin(1)))*(HorizontalConrathProfile(cmin(1) + 1,iLat,iBlock) - &
       ...
       if (iproc .eq. 1 .and. ilat .eq. 1 .and. ilon .eq. 5) write(*,*) HorizontalConrathProfile(imin(1),iLat,iblock),&
            ConrathDistribution(iLon,iLat,iBlock) 
    else
       tautot = 0.2
       conrnu = 0.03
    endif
...
!stop
    ...
    do L=2,l_nlayrad
!  mbars pressure interpolation and masking done!
       L1=L-1
