...
!     write(*,*) GWAccel(1,1,:,1)
  ...
  subroutine  calc_lowatmosrad(iblock,iLat,iLon,L_LAYERS,L_LEVELS,&
    ...
    REAL :: DustTime(nDustTimes),invDDiff
    ...
    real :: FMNETV(LL_NLAYRAD), diffvt,tautot, tDiff(nDustTimes)
    real :: fluxupv(LL_NLAYRAD), fluxdnv(LL_NLAYRAD), NFLUXTOPV

    real :: fluxvd(LL_LAYERS),HEATING(LL_LAYERS),TOTAL(LL_LAYERS),rtime
    real :: fluxid(LL_LAYERS),GREENHOUSE(LL_LAYERS),XLTECORRECTION(LL_LAYERS)

    integer :: ngwv(L_NSPECTV),ialt,imin(1)
...
!stop
 if (UseDustDistribution) then
    ...
    CALL DUSTPROFILE(PLEV(L_LEVELS),PTROP,PLEV,TAUCUM,TAUREF,L_LEVELS,tautot)
...
!stop
  ...
  subroutine dustprofile(PSF,PTROP,PLEV,TAUCUM,TAUREF,L_LEVELS,TauTot)
