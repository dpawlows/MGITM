...
! ***********************************************************************************
!  Paramater adjustmenst for Co2 cooling crossover; near IR heating crossover, and
!  aerosol distribution (and resulting LATM heating)  ; Septmeber 7, 2011  (S.Bougher)
!  CONRNU = 0.003   ! Deep mix value  ~50 km half-height
!  TAUTOT  = 0.5    ! moderate (global average)
!*  PRAD  = 1.0E-5   ! top of LTE near IR heating radcode code (mb). ~108 km
!*  PRAD  = 1.0E-6   ! top of LTE near IR heating radcode code (mb). ~125 km
!  PLONG = 0.4      ! bottom of NLTE CO2 cooling radcooling code  (Pa) 
! ***********************************************************************************
...
!      real, parameter :: CONRNU = 0.003   ! Deep mix value  ~50 km half-height
!     real, parameter :: CONRNU = 0.03    ! Standard value  ~25 km half-height
!     real, parameter :: CONRNU = 0.5     ! Shallow value   ~10 km half-height

!   RPTAU   :  Reference Pressure optical depth;  6.1 mbar for now
  real, parameter :: RPTAU  = 6.1

! Global mean dust opacity: do not set to 0.0 or less
! real, parameter :: TAUTOT  = 0.3 !minimal  (global average)
Logical :: UseDustDistribution = .False.
!  real :: TAUTOT  = 0.5 !moderate (global average)

! Top of near-IR shortwave calculation for lower atmosphere radiation code(mbars);
! Fixed by correlated-k coefficients included in the RT code.
! real, parameter :: PRAD  = 1.0E-5 !mb
  real, parameter :: PRAD  = 1.0E-6 !mb

! Top of the LTE longwave calculation for lower atmosphere radiation code(PASCALS)
! and where NLTE CO2 15-micron radcooling begins
! real, parameter :: PLONG = 1.0E-1 ! Pa  (also 1.0  ubar)
  real, parameter :: PLONG = 4.0E-1 ! Pa  (also 4.0  ubar)
! real, parameter :: PLONG = 1.0    ! Pa  (also 10.0 ubar)
