...
subroutine init_msis
 ...
 real :: DustLatitude(ndustlats),TempDust(ndustlats),Temp(ndustlats+6)
  real :: rlat, invLatDiff, LatFind, Latdiff, Dust,templat(ndustlats)
  ...
  do iBlock = 1, nBlocks
     ...
     !/
     if (UseDustDistribution) then !Read in dust information                                                  
        open(unit=iInputUnit_,file='dust.dat')
        notstarted = .True.
        do while (notstarted)
