...
! New Method, A. Ridley fixes (Spring 2012)
! Further Fix to tmp3 definition (second usage), S. Bougher (Spring 2012)
...
!          tmp3(1:nLons,1:nLats,0:nAlts+1) = &
!             Rho(1:nLons,1:nLats,1:nAlts,iBlock)/Gamma(1:nLons,1:nLats,1:nAlts,iBlock)

           tmp3(1:nLons,1:nLats,0:nAlts+1) = &
              Rho(1:nLons,1:nLats,0:nAlts+1,iBlock)/Gamma(1:nLons,1:nLats,0:nAlts+1,iBlock)
...
!     write(*,*) '==========> Now Exiting Neutral Friction Calculation!!'
  ...
  if(UseViscosity)then
     ...
     call calc_conduction(iBlock, &
          ...
          Rho(1:nLons, 1:nLats,-1:nAlts+2, iBlock), &
