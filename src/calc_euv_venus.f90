 ...
 if (UseEUVData) then
    ...
    do N=1,Num_WaveLengths_High
       if (UseFluxAtPlanet) then
         ...
         !!! Explicti correction for Venus heliocenctric distance : YYMMDD = 2015.04.19
         !!!  using fismdaily.dat7
         !!! Where Rmars = 1.49AU; Rmars**2 = 2.2201
         !!! Where Revenus = 0.72AU; Rvenus**2 =  0.5184 
             Flux_of_EUV(N) = Timed_Flux(N)*wvavg(N)*1.0e-10/(6.626e-34*2.998e8)* &
                  (2.2201/0.5184)*/(SunOrbitEccentricity**2)
         ! --------------------------------------------------------------------------
       else
          Flux_of_EUV(N) = Timed_Flux(N)*wvavg(N)*1.0e-10/(6.626e-34*2.998e8) &
