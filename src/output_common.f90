...
cTime = cTimeSave
  ...
  case ('3DTHM')
   ...
   ! nvars_to_write = 18
     ...
     ! 1  (LowerAtmos) or (QnirTOT)
     ! 1  (UserData 40: Horiz Adiabatic)
     ! 1  (UserData 41: Horiz Hydrodynamic)

     ! 1  (UserData 42: Vertical Adiabatic)
     ! 1  (UserData 43: Vertical Hydrodynamic)
     ! 1  (UserData 44: Viscosity Heating )
     ! 1  (gwdrag-1 : udrag (m/s/sol))
     ! 1  (gwdrag-2 : vdrag (m/s/sol))
     ! 1  (gwiheat  : gwheat_ir(K/sol))
     ! 1  (gwdheat  : gwheat_dif(K/sol))
...
contains
  ...
  subroutine output_header
    ...
    if (cType(3:5) == "THM") then
       ...
       write(iOutputUnit_,"(I7,A1,a)")  6, " ", "EUV Heating (K/sol) "
       write(iOutputUnit_,"(I7,A1,a)")  7, " ", "Conduction  (K/sol)"
...
!      write(iOutputUnit_,"(I7,A1,a)")  9, " ", "Lower Atmos Heating/Cool (K/sol)"
       ...
       write(iOutputUnit_,"(I7,A1,a)")  17, " ", "Irreversible GW Heating (K/sol) "
       write(iOutputUnit_,"(I7,A1,a)")  18, " ", "Differential GW Heating (K/sol) "
       write(iOutputUnit_,"(I7,A1,a)")  19, " ", "Net GW Heating (K/sol) "
       write(iOutputUnit_,"(I7,A1,a)")  20, " ", "GW_flux_tot TEST (m2/s2)"
       write(iOutputUnit_,"(I7,A1,a)")  21, " ", "GW_brunt TEST (1/s)"
     ...
     do iLat=-1,nLats+2
        ...
        do iLon=-1,nLons+2
           ...
           write(iOutputUnit_)          &
                ...
                Latitude(iLat,iBlock),                &
                Altitude_GB(iLon,iLat,iAlt,iBlock),   &
! I have added rho and cp to the output files
...
!               UserData3D(iiLon,iiLat,iiAlt,43,iBlock), &
! Viscous Heating
!               UserData3D(iiLon,iiLat,iiAlt,44,iBlock)
!
! ------------------------------------------------------------------------------
...
! GWHEAT_IR and GWHEAT_DIF Terms from EGWD2 (47-48) : K/sol
                     GWIHEAT(iiLon,iiLat,iiAlt,iBlock)*88775., &
                     GWDHEAT(iiLon,iiLat,iiAlt,iBlock)*88775., &
! GW test terms
		GW_net_heating(iiLon,iiLat,iiAlt,iBlock), &
		GW_flux_tot(iiLon,iiLat,iiAlt,iBlock), &
		GW_brunt(iiLon,iiLat,iiAlt,iBlock), &
		GW_flux(iiLon,iiLat,iiAlt,iBlock), &
		GW_drag(iiLon,iiLat,iiAlt,iBlock), &
		GW_var_tot(iiLon,iiLat,iiAlt,iBlock)
...
subroutine output_3dglo(iBlock)
  ...
  use ModGITM
  use ModInputs
!  iENOUV is calculated for NO* nighglow volume emission rate (ph/cm3/sec)
!  S. Bougher (170427)
...
!                   Latitude(iLat,iBlock),                &
!                   Altitude_GB(iLon,iLat,iAlt,iBlock),   &
!                   vEmissionRate(iiLon,iiLat,iiAlt,iENOUV_,iBlock), &
!                   Emission(iiLon,iiLat,iiAlt,iENOUV_,iBlock)
...
!        Tmp = NDensityS(0:nLons+1,0:nLats+1,iAlt,iSpecies,iBlock)/NDensity(0:nLons+1,0:nLats+1,iAlt,iBlock)
     ...
     Vars(iOff) = itemperature(1,1,ialt,iblock)!inter(Tmp,iiLon,iiLat,rlon,rlat)
